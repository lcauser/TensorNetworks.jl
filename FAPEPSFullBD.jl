...
psi, energy = fullupdate(psi, H, 0.1, sh; maxdim=2, maxiter=500, miniter=200, chi=1, chieval=16, saveiter=100, update_tol=1e-7, cutoff=1e-4)
if s > 0
    states = [["dn" for i = 1:N] for j = 1:N]
    states[1][1] = "up"
    psi2 = productPEPS(sh, states)
    psi2, energy2 = fullupdate(psi2, H, 0.1, sh; maxdim=1, maxiter=10000, miniter=100, chi=1, saveiter=100)
    psi2, energy2 = fullupdate(psi2, H, 0.1, sh; maxdim=2, maxiter=500, miniter=200, chi=4, chieval=16, saveiter=100, update_tol=1e-7, cutoff=1e-4)
    psi = energy2 > energy ? psi2 : psi
end

# Evolve fully
psi, energy = fullupdate(psi, H, 0.01, sh; maxdim=2, maxiter=10000, miniter=200, chi=4, saveiter=100)
psi, energy = fullupdate(psi, H, 0.01, sh; maxdim=3, maxiter=10000, miniter=200, chi=9, saveiter=100)
psi, energy = fullupdate(psi, H, 0.01, sh; maxdim=4, maxiter=10000, miniter=200, chi=1, saveiter=100)
psi, energy = fullupdate(psi, H, 0.001, sh; maxdim=4, maxiter=1000, miniter=200, chi=16, saveiter=100)
end
