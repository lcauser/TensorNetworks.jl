...
function lindblad(N, omega, gamma, kappa, s, sh, E=0)
    # Create the MPO
...
function lindblad2(N, omega, gamma, kappa, s, sh, E=0)
    # Create the MPO
    A1 = -1im*omega*op(sh, "xid") - 0.5*kappa*op(sh, "puid") - 0.5*gamma*op(sh, "pdid")
    A2 = 1im*omega*op(sh, "idx") - 0.5*kappa*op(sh, "idpu") - 0.5*gamma*op(sh, "idpd")
    A3 = kappa*exp(-s)*op(sh, "s-s-") + gamma*exp(-s)*op(sh, "s+s+")
...
if s > 0
    for dt in [0.01]
        ...
        psi2, energy2 = tebd(psi2, oplist, sh, dt, 1.0, save, [TEBDNorm(1e-5)]; maxdim=4)
        global psi2 = psi2
...
saveDirect = string(direct, "TEBD/left/gamma = ", gamma, "/omega = ",
                    omega, "/N = ", N, "/")
if !isdir(saveDirect)
    mkpath(saveDirect)
end
