...
function qjmc_simulation(psi::MPS, Hs::OpList, jumpops::OpList, st::Sitetypes,
    ...
    if update != nothing
        originaljumpops = deepcopy(jumpops)
        jumpops = qjmc_update_rates(psi::MPS, originaljumpops::OpList, st::Sitetypes)
    end
    #escapeops, gates = qjmc_gates(st, H, jumpops, dt; kwargs...)
    ...
    for i = 1:steps
        # Update the jump operators
        if update != nothing
            jumpops = qjmc_update_rates(psi::MPS, originaljumpops::OpList, st::Sitetypes)
            escapeops, gates2 = qjmc_gates(st, H, jumpops, dt; kwargs...)
        end

        # Calculate the rates
        rates = real(inner(sh, psi, escapeops, psi))
...
function QJMCOperators(oplist::OpList, st::Sitetypes)
    return QJMCOperators([], [], oplist, st)
end
